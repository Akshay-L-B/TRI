...
const initialFeedbackData = {
  instructorAvailability:
    "How accessible was the instructor for questions and assistance?",
  realWorldExamples:
    "Did real-world examples enhance your understanding of the course?",
  relevantCourseContent:
    "Was the course content relevant to your learning or professional needs?",
  ...
  const submitFeedback = async () => {
    try {
      ...
      const response = await fetch(
        `/api/coursefeedback?StudentID=${StudentID}&CourseCode=${CourseCode}`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        toast.success(
          "Course Feedback for course ${CourseCode} Submitted Sucessfully!",
          {
            position: "top-center",
            autoClose: 1500,
