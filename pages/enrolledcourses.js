...
const EnrolledCourses=({user})=>{
 ...
 const [selectedSchedule, setSelectedSchedule] = useState(null)
  ...
  const handleEitClick = async (CourseCode, CourseTitle,CourseDropDeadline) => {
          ...
          <div className="flex flex-col mt-6">
            <div className="-mx-4 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
              ...
              {dummyCourses.map((course) => (
                <div key={course.id} className='relative overflow-hidden border border-teal-500 hover:border-2 rounded-lg transition-all'>
                  <Link href='#'>
                    <img
                      src={course.imageUrl}
                      alt='course cover'
                  ...
                  <div className='p-3'>
                    <p className='text-lg font-semibold line-clamp-2 mb-2'>{course.title}</p>
                    <span className='italic text-sm text-gray-500'>{`${course.CourseLanguage}-${course.CourseLevel}`}</span>
                    <div className="mt-2 flex space-x-4">
                      <button onClick={() => handleEditClick(course.id)} className='text-teal-500 hover:bg-teal-500 hover:text-white px-3 py-2 border border-teal-500 rounded-md transition-all duration-300'>
                        See Course Contents
                      </button>
                    </div>
                  </div>
